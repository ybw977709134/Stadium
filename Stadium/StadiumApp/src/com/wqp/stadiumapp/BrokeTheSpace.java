...
public class BrokeTheSpace extends Activity implements IXListViewListener, OnClickListener{
	private static String TAG="BrokeTheSpace";
	private ImageView brokethespace_back_arrow;
	private XListView brokethespace_listview;
	private List<Map<String,Object>> mListViewData;
	private NotSpeechSecretAndBrokeTheSpaceAdapter mBrokeTheSpaceAdapter;
	...
	private Handler mHandler=new Handler(){
		...
		public void handleMessage(android.os.Message msg) {
			...
			case 0x11://从线程中查询的数据集返回此处 
				...
				mBrokeTheSpaceAdapter.notifyDataSetChanged();//更新ListView适配器的数据  
				break;
			case 0x22://标识获取到的数据为空 
				Toast.makeText(BrokeTheSpace.this, "没有找到哦!", 0).show();
			case 0x33://标识上拉加载时已经没有数据了
				Toast.makeText(BrokeTheSpace.this, "亲,没有了哦!", 0).show();	
			default:
				break;
			} 
		};
	...
	protected void onCreate(Bundle savedInstanceState) { 
		...
		setContentView(R.layout.broke_the_space);
		brokethespace_back_arrow=(ImageView) findViewById(R.id.brokethespace_back_arrow);
		brokethespace_listview=(XListView) findViewById(R.id.brokethespace_listview);
		
		//为XListView绑定上拉刷新事件,并进行配置
		brokethespace_listview.setPullLoadEnable(true);
		brokethespace_listview.setPullRefreshEnable(false);
		brokethespace_listview.toggleHeadView=true;//不使用头部刷新功能
		
		//为返回按钮绑定事件
		brokethespace_back_arrow.setOnClickListener(this); 
	...
	public void initXListView(){
		...
		//给适配器填充数据
		mBrokeTheSpaceAdapter=new NotSpeechSecretAndBrokeTheSpaceAdapter(
				BrokeTheSpace.this, 
				getListViewData(), 
				R.layout.brokethespace_listview_item, 
				new String[]{"Title","SGContent"}, 
				new int[]{R.id.brokethespace_title,R.id.brokethespace_content});
		//设置适配器的数据
		brokethespace_listview.setAdapter(mBrokeTheSpaceAdapter);
		//为XListView绑定事件
		brokethespace_listview.setOnItemClickListener(new OnItemClickListener(){ 
			@Override
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) { 
//				Toast.makeText(BrokeTheSpace.this,position+"", 0).show(); 
				Intent intent=new Intent(BrokeTheSpace.this,BrokeTheSpaceDetails.class);
				Bundle extras=new Bundle();
				extras.putSerializable("GetGuideBean", WebDataSet.get(position-1));
				intent.putExtras(extras);
				BrokeTheSpace.this.startActivity(intent);//跳转到爆料空间的详情界面
			} 
		});
...
//			Toast.makeText(BrokeTheSpace.this, "没有找到哦", 0).show();
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.brokethespace_back_arrow://点击了返回按钮
			finish();
			break; 
		} 
	}
	...
	public void conditionQuery() { 
			mProgressDialog=new ProgressDialog(BrokeTheSpace.this);
	...
	/** 停止ListView底部刷新 */
	private void onLoad() {
		brokethespace_listview.stopRefresh();
		brokethespace_listview.stopLoadMore();
		brokethespace_listview.setRefreshTime("刚刚");
		brokethespace_listview.invalidate();
	}
