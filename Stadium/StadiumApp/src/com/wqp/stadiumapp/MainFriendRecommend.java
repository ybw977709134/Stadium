...
public class MainFriendRecommend extends Activity implements IXListViewListener, OnClickListener{ 
...
//	private int sporttype;//用户在主界面点击的运动类型
	...
	private Handler mHandler=new Handler(){
		...
		public void handleMessage(android.os.Message msg) {
			...
			case 0x22://标识获取到的数据为空 
				Toast.makeText(MainFriendRecommend.this, "没有找到哦!", 0).show();
				break;
			case 0x33://标识上拉加载时已经没有数据了
				Toast.makeText(MainFriendRecommend.this, "亲,没有了哦!", 0).show(); 
				break;  
			};
		};
	};
...
//			Toast.makeText(FriendRecommendType.this, position+"", 0).show(); 
			...
			//跳转到加入后页面详细界面
			Intent itemIntent=new Intent(MainFriendRecommend.this,JoinActDetails.class);
			Bundle zzextras=new Bundle();
			zzextras.putSerializable("GetActInfoBean", WebDataSet.get(position-1));
			itemIntent.putExtras(zzextras);
			MainFriendRecommend.this.startActivity(itemIntent);//携带数据跳转到JoinActDetails应战详情页面 
		} 
	}
	...
	public void conditionQuery() { 
			if(!ToolsNavigate.isNetworkAvailable(MainFriendRecommend.this)){ return;}//判断当前网络是否可用
			mProgressDialog=new ProgressDialog(MainFriendRecommend.this);
...
//						Log.i(TAG,"用户点击的查询的参数是:SportType="+sporttype);
						 ...
						 if(results!=null){
							 ...
							 //获取到当前登录的用户已经参加的活动信息,返回的只是当前用户所参加活动编号集合
							 List<Integer> ids=WebGetUserOnAct.getGetUserOnActData(UserGlobal.UserID); 
							 if(ids!=null){
								 for (GetActInfoBean getActInfoBeanS : results) {
									 Log.i("MainFreindRecommend","从服务端加载到当前用户已经加入应战的活动ID集合当中是否包含活动ID为："+getActInfoBeanS.getActid()+" 的状态是:"+ids.contains(getActInfoBeanS.getActid()));							
									 getActInfoBeanS.setImgstate(ids.contains(getActInfoBeanS.getActid()));//判断当前活动的ID号是否是在用户已经参加活动的ID号范围之类
								 }
							 } 
						 } 
//						 if(results!=null){
