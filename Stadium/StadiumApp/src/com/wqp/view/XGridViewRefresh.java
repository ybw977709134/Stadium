...
//		// int top = getHeight();
//		// params.topMargin
//		// =getHeight();//在这里getHeight()==0,但在onInterceptTouchEvent()方法里getHeight()已经有 值了,不再是0;
//		// getHeight()什么时候会赋值,稍候再研究一下
//		// 由于是线性布局可以直接添加,只要AdapterView的高度是MATCH_PARENT,那么footer view就会被添 加到最后,并隐藏
...
//	 * false)则由PullToRefreshView 的子View来处理;否则由下面的方法来处理(即由PullToRefreshView自己来处
//
//理)
//	 */
//	@Override
...
//	 * event.getRawY(); // onInterceptTouchEvent已经记录 // mLastMotionY = y; break;
//	 * case MotionEvent.ACTION_MOVE: int deltaY = y - mLastMotionY; if
//	 * (mPullState == PULL_DOWN_STATE) { // PullToRefreshView执行下拉
...
//	 * >= 0) { // 开始刷新 headerRefreshing(); } else { // 还没有执行刷新，重新隐藏
//	 * setHeaderTopMargin(-mHeaderViewHeight); } } else if (mPullState ==
//	 * PULL_UP_STATE) { if (Math.abs(topMargin) >= mHeaderViewHeight +
//	 * mFooterViewHeight) { // 开始执行footer 刷新 footerRefreshing(); } else { //
//	 * 还没有执行刷新，重新隐藏 setHeaderTopMargin(-mHeaderViewHeight); } } case
...
//					return false;
//				}
//				// 最后一个子view的Bottom小于父View的高度说明mAdapterView的数据没有填满父 view,
...
//		// 这里对上拉做一下限制,因为当前上拉后然后不释放手指直接下拉,会把下拉刷新给触发了,感谢网友 yufengzungzhe的指出
