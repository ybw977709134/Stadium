...
public class XListView extends ListView implements OnScrollListener {
	...
	/**是否把头部刷新视图添加到ListView当中  true表示添加，返之false*/
	public boolean toggleHeadView=true;
	...
	public void setAdapter(ListAdapter adapter) {
		...
		if (mIsFooterReady == false) {
			...
			mFooterView.setVisibility(View.INVISIBLE);//显隐藏底部
	...
	private void updateFooterHeight(float delta) {
		...
		mFooterView.setVisibility(View.VISIBLE);//显示底部
...
//		LinearLayout ll = (LinearLayout) getChildAt(1);
		...
		for(int i=0;i<len;i++){ 
			//这里对子视图ViewPager进行判断处理操作
			View vv=getChildAt(i); 
			if(vv.getId()== R.id.home_fragment_ids){
				Rect rect = new Rect();
				...
				if (rect.contains((int) ev.getX(), (int) ev.getY())) {
					// 在框里面
					 ...
					 vt.addMovement(ev);
//					 int velocity=vc.getScaledMaximumFlingVelocity();
					 mMinSlop=vc.getScaledTouchSlop(); //求滑动的最小值
					switch (ev.getAction()) {
					case MotionEvent.ACTION_DOWN:
						mX=ev.getX(); 
						break;
					...
					case MotionEvent.ACTION_MOVE:
						if(Math.abs(ev.getX()-mX)>mMinSlop){
							return false;//把事件分发下去
						} 
						break;
					} 
				}  
